...
void QQuickGraphs2DView::updateAxisTickers()
{
    ...
    m_axisTickerVertical->setSpacing(m_axisTickerVertical->height() / 20);
...
void QQuickGraphs2DView::updateBarYAxis(QBarCategoryAxis *axis, const QRectF &rect)
{
    // TODO: No hard-coded item amount
    int categoriesCount = 20 + 1;
    // See if we need more text items
    ...
    for (int i = 0;  i < categoriesCount; i++) {
        ...
        float posX = rect.x();
        textItem->setX(posX);
        float posY = rect.y() + rect.height() - fontSize * 1.0 - (((float)textIndex) / (categoriesCount-1)) *  rect.height();
        textItem->setY(posY);
...
void QQuickGraphs2DView::updateBarSeries(QBarSeries *series)
{
    ...
    if (QBarCategoryAxis *axisY = qobject_cast<QBarCategoryAxis*>(series->axisY())) {
